...
<form id="form1" name="form1" method="post" action="">
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="5" bordercolor="#FFFFFF" bgcolor="#FFFFCC">
    <tr>
      <td width="50%">Technicien</td>
      <td width="50%"><select name="t1" id="t1">
 <?php 
  include("connection.php");
  $sql= mysql_query("select* from technicien");
  ...
  { $num=$res["num"];
	...
	$type=$res["type"];
  ?>
<option value="<?php echo"$cin"; ?>"><?php echo"$nom"; ?></option>
<?php
}
?>
      </select>      </td>
...
<table width="100%" border="1" align="center" cellpadding="0" cellspacing="3">
  ...
  $dd=$_POST["t2"];
  $df=$_POST["t3"];
  $sql= mysql_query("select* from tache where ncin='$ncin' and date between '$dd' and '$df'");
  while($res=mysql_fetch_array($sql))
  { $code=$res["code"];
